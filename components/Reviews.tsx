	...
	return (
		<ul className="flex flex-col gap-4 my-10">
			{reviews?.slice(0, 9).map(({ id, username, title, content, imgSrc }) => (
				<li
					key={id}
					className="px-4 py-4 border border-gray-200 rounded-xl outline-rose-200 outline-2 outline-offset-2 hover:outline active:outline cursor-pointer transition-all sm:px-6 md:px-8">
